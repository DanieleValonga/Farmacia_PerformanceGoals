...
function CardPostagem({ type, title, description, validade }: CardPostagemProps) {
  const cardClass =
    type === 'beleza' && title === 'Dipirona' ? 'card-postagem dipirona' :
    type === 'higiene' && title === 'Tylenol' ? 'card-postagem tylenol' :
    type === 'farmaco' && title === 'Absorvente' ? 'card-postagem absorvente' :
    'card-postagem';
